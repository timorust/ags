...
const Cards = ({ item }) => {
  ...
  return (
    <div className='dark:bg-slate-900 dark:text-white dark:border card bg-base-100 w-full sm:w-1/2 md:w-1/3 lg:w-1/4 xl:w-1/3 2xl:w-1/4 shadow-xl transform transition-transform duration-700 ease-out hover:scale-105 rounded-lg m-2'>
      {/* עיצוב תמונה רספונסיבי */}
      <figure className='overflow-hidden w-full h-48 sm:h-56 md:h-64 lg:h-72'>
        <img
          src={item.image}
          alt={t(item.name)}
