...
function About() {
    ...
    const images = ['/about4.jpg', '/about5.png', '/about2.jpg', '/about3.jpg'];
    ...
    return (
        <>
            ...
            <div className='max-w-screen-lg container mx-auto px-4 md:px-12 mb-8'>
                ...
                <div className='relative w-full h-48 sm:h-64 lg:h-80 xl:h-96 overflow-hidden rounded-lg shadow-lg flex justify-center items-center'>
                    {images.map((image, index) => (
                        <img
                            key={index}
                            src={image}
                            srcSet={`${image}?w=320 320w, ${image}?w=480 480w, ${image}?w=768 768w, ${image}?w=1024 1024w`}
                            sizes='(max-width: 640px) 100vw, (max-width: 1024px) 80vw, (max-width: 1280px) 70vw, 60vw'
                            alt={`Image ${index + 1}`}
                            className={`absolute inset-0 w-full h-full object-cover rounded-lg transition-opacity duration-500 ${index === imageIndex ? 'opacity-100' : 'opacity-0'}`}
                        />
                    ))}
                </div>

                <div className='bg-gray-100 dark:bg-blue-300 p-4 rounded-md shadow-md mb-8 mt-8'>
                    <div className='text-lg text-gray-500 marquee-container overflow-hidden w-full'>
                        <div
                            className='marquee-text inline-block pl-full'
